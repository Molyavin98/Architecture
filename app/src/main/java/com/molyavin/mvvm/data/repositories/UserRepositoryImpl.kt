package com.molyavin.mvvm.data.repositories

import com.molyavin.mvvm.data.storage.DBSharedPreference
import com.molyavin.mvvm.domain.models.UserInfo

private const val KEY_PHONE = "Phone"
private const val KEY_PASSWORD_ONE = "PasswordOne"
private const val KEY_PASSWORD_TWO = "PasswordTwo"
private const val DEFAULT = "null"

class UserRepositoryImpl(private val sharedPreferences: DBSharedPreference) :
    UserRepository {
    override fun saveData(userInfo: UserInfo) {
        sharedPreferences.saveData(KEY_PHONE, userInfo.phone)
        sharedPreferences.saveData(KEY_PASSWORD_ONE, userInfo.passwordOne)
        sharedPreferences.saveData(KEY_PASSWORD_TWO, userInfo.passwordTwo)
    }
    override fun readData(): UserInfo = UserInfo(
        phone = sharedPreferences.getData(KEY_PHONE) ?: DEFAULT,
        passwordOne = sharedPreferences.getData(KEY_PASSWORD_ONE) ?: DEFAULT,
        passwordTwo = sharedPreferences.getData(KEY_PASSWORD_TWO) ?: DEFAULT,
    )
}